...
const seedData = async () => {
  try {
    ...
    if (!tables.rows.some(table => table.table_name === 'users')) {
    ...
    await query(
      ...
      ['admin', 'admin@themesbrand.com', adminPassword, 'Admin', 'User', 'admin', true, true]
    );
    ...
    await query(
      `INSERT INTO users (username, email, password_hash, first_name, last_name, role, is_active, email_verified)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT (email) DO NOTHING`,
      ['testuser', 'testuser@example.com', userPassword, 'Test', 'User', 'user', true, true]
    );

    // Add demo user for frontend demo
    const demoPassword = await bcrypt.hash('demo123', 12);
    await query(
      `INSERT INTO users (username, email, password_hash, first_name, last_name, role, is_active, email_verified)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT (email) DO NOTHING`,
      ['demo', 'demo@example.com', demoPassword, 'Demo', 'User', 'user', true, true]
    );
